...
def launch_setup(context, *args, **kwargs):
   ...
   area_memory = LaunchConfiguration('area_memory')

   path_max_count = LaunchConfiguration('path_max_count')
   ...
   zed_wrapper_node = Node(
       ...
       parameters=[
           ...
           # Overriding
           {
               ...
               'pos_tracking.area_memory': area_memory,
               'pos_tracking.path_max_count': path_max_count,
               'pos_tracking.qos_depth': qos_depth
           },
       ...
       [
           SetEnvironmentVariable(name='RCUTILS_COLORIZED_OUTPUT', value='1'),
            DeclareLaunchArgument(
               'area_memory',
               default_value='true',
               description='Disables loop closure computation, but Pose topic for global VSLAM still published (but now pure VIO)',
               choices=['true', 'false']),           
           DeclareLaunchArgument(
               'camera_name',
           ...
           DeclareLaunchArgument(
               'serial_number',
               default_value='0',
               description='The serial pos_tracking_enablednumber of the camera to be opened. It is mandatory to use this parameter in multi-camera rigs to distinguish between different cameras.'),
            DeclareLaunchArgument(
               'path_max_count',
               default_value='-1',
               description='maximum size of the visualization of the pose over time. By default it is infinite. We should make this finite.'),
           DeclareLaunchArgument(
               'publish_urdf',
               default_value='true',
